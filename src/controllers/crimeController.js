...
const createCrime = async (req, res) => {
  try {
    ...
    const populatedCrime = await Crime.findById(savedCrime._id).populate(
      "officer",
      "name badgeNumber department assignedCases"
    );
    ...
    res.status(500).json({
      success: false,
      message: "Failed to create crime report",
      error: error.message
    });
  }
};
const getAllCrimes = async (req, res) => {
  try {
    ...
    if (startDate || endDate) {
      ...
      if (startDate) {
        filters.incidentDate.$gte = new Date(startDate);
      }

      if (endDate) {
        filters.incidentDate.$lte = new Date(endDate);
      }
    }
...
const updateCrime = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to update crime report",
      error: error.message
    });
  }
};
...
const getCrimesByCity = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch city-wise crime stats",
      error: error.message
    });
  }
};
...
const getCrimesByType = async (req, res) => {
  try {
    ...
    res.status(500).json({
      success: false,
      message: "Failed to fetch crime type stats",
      error: error.message
    });
  }
};
...
const assignCrimeToOfficer = async (req, res) => {
  ...
  try {
    ...
    if (!crimeId || !officerId) {
      throw new Error("crimeId and officerId are required");
    }
  ...
  if (prevOfficer) {
    ...
    const updatedCrime = await Crime.findById(crimeId).populate(
      "officer",
      "name badgeNumber department assignedCases"
    );
    ...
    res.status(200).json({
      success: true,
      message: "Crime assigned to officer successfully",
      data: updatedCrime
    });
  } catch (error) {
